...
public class DeviceDeactivatePresenter extends BasePresenter<DeviceView> {
    public void deviceDeactivateGateway(long deviceId){
        AppApplication.appFutureImpl.deviceDeactivateGateway(deviceId, tag, new FutureListener() {
            @Override
            public void onStart() {
                super.onStart();
                getView().showLoading();
